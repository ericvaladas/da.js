function Crypto(seed, key, keySaltSeed) {
  this.salts = [
    [  // salt 00
  ...
  this.salt = function() {
    return this.salts[this.seed]
  }

  this.generateKey = function(a, b) {
    var key = ""
    var saltIndex = 0
    for (var i = 0; i < 9; i++) {
        saltIndex = (i * (9 * i + int32(b) * int32(b)) + a) % this.keySalt.length
        key += this.keySalt[saltIndex]
    }

    return key
  }

  this.generateKeySalt = function(seed) {
    var keySalt = this.getHashString(seed)
    keySalt = this.getHashString(keySalt)
  ...
  this.getHashString = function(value) {
    return md5(value)
  }
  ...
  if (keySaltSeed != undefined) {
      this.keySalt = this.generateKeySalt(keySaltSeed)
  }
}
...
function NexonCRC16() {
  ...
  this.calculate = function(buffer, index, length) {
    if (index == undefined) {
      index = 0
    }
